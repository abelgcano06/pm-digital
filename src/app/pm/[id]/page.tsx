...
type PMTaskTemplate = {
  ...
  reason: string;
  order: number;
  hasImage?: boolean; // viene del backend
};
...
export default function PMExecutionPage({
  ...
  // 👇 Nuevo: foto seleccionada por cada tarea (taskId -> File)
  ...
  const currentTaskTemplate =
    pmTemplate && totalTasks > 0
      ? pmTemplate.tasks[Math.min(currentIndex, totalTasks - 1)]
      : null;
  ...
  const handleNext = () => {
    ...
    if (currentIndex < pmTemplate.tasks.length - 1) {
  ...
  const canFinish = useMemo(() => {
    ...
    for (const t of tasksExec) {
      const template = pmTemplate.tasks.find((x) => x.id === t.taskId);
      if (!template) continue;
      const needsMeasure = isMeasurementTask(template);
  ...
  // 👇 Nuevo helper: guardar la foto seleccionada para una tarea
  const handlePhotoChange = (taskId: string, file: File | null) => {
    setPhotosByTask((prev) => ({
      ...prev,
      [taskId]: file,
    }));
  };
          ...
          <section className="pm-card pm-card-main">
            ...
            {currentTaskTemplate && currentTaskExec && (
              <div className="pm-task-panel">
                ...
                <div className="pm-controls-box">
                  ...
                  {currentTaskTemplate && (
                    <div className="pm-photo-block">
                      ...
                      <div className="pm-photo-row">
                        <button
                          type="button"
                          className="pm-btn pm-btn--secondary"
                          onClick={() => {
                            const input = document.getElementById(
                              `photo-input-${currentTaskTemplate.id}`
                        ...
                        {photosByTask[currentTaskTemplate.id] && (
                          <span className="pm-photo-label">
                            Foto seleccionada:{" "}
                            {photosByTask[currentTaskTemplate.id]?.name}
                          </span>
                        )}
                      </div>

                      <input
                        id={`photo-input-${currentTaskTemplate.id}`}
                        type="file"
                        accept="image/*"
                        capture="environment"
                        style={{ display: "none" }}
                        onChange={(e) => {
                          ...
                          handlePhotoChange(currentTaskTemplate.id, file);
                        }}
                      />
                    </div>
                  )}
                ...
                {/* 4) PROGRESO Y FLAGS ABAJO DEL TODO */}
                ...
                {/* Botón FINALIZAR abajo del progreso */}
